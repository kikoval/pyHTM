...
class BaseExplorer:
    ...
    def __init__(self, getOriginalImage, getFilteredImages, getImageInfo,
        ...
        Args:
            ...
            seed: Seed for the random number generator. A specific random
                  number generator instance is always created for each
                  explorer, so that they do not affect each other
        """
    ...
    def isBlank(self, fallOffObject, position=None):
        """Return True if the enabled region of the image specified by the
        current position is blank.

        Args:
            fallOffObject: If True, the image is considered blank only if the
                           mask is entirely black. Otherwise, it is considered
                           blank if any of mask is black
